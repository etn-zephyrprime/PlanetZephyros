// Wires up V7's multi-currency support on an already-deployed mainnet instance: coreToken/
// swapRouter (same proven values already live on PremiumSubscription and V3/V4/V5/V6), 9
// whitelisted ERC20 payment tokens with a starting minSubnamePricePerYear floor each, and
// v3PoolForToken for the 4 tokens whose real liquidity lives in an ElectroSwap V3 pool instead of
// the plain V2 swapRouter path. Run with the marketplace's OWNER account active in MetaMask —
// every call here is owner-only. Run AFTER deployMarketplaceV7_mainnet_remix.ts, with
// MARKETPLACE_ADDRESS below set to the address it printed.
//
// Same values V6's own configureMarketplaceV6Currencies_remix.ts already set and verified live —
// none of this changed for V7 (the only difference between V6 and V7 is the domain-locking/
// subname-protection mechanism; currency/liquidity config is identical). Re-run here rather than
// copied automatically because none of it carries over from one deployment to the next (no upgrade
// path, same as every version so far) — this script exists specifically to re-do that one-time
// setup on the new contract.
//
// Token liquidity last checked live 2026-09-15 against the SAME router (0x072D...FFd7)/its factory
// (0x203D...d15b) — re-verify before running if meaningful time has passed since then:
...
const MARKETPLACE_ADDRESS: string = '' // fill in with the address deployMarketplaceV7_mainnet_remix.ts printed

// Same values already verified live on PremiumSubscription and V3/V4/V5 (setupMainnetCoreBuyback_remix.ts).
