// Wires up V6's multi-currency support on an already-deployed mainnet instance: coreToken/
// swapRouter (same proven values already live on PremiumSubscription and V3/V4/V5), 9 whitelisted
...
// is owner-only. Run AFTER deployMarketplaceV6_mainnet_remix.ts, with MARKETPLACE_ADDRESS below
// set to the address it printed.
//
// Same values V5's own configureMarketplaceV5Currencies_remix.ts already set and verified live —
// none of this changed for V6 (the only difference between V5 and V6 is the _fulfillSubname
// availability-check fix; currency/liquidity config is identical). Re-run here rather than copied
// automatically because none of it carries over from one deployment to the next (no upgrade path,
// same as every version so far) — this script exists specifically to re-do that one-time setup on
// the new contract.
//
...
const MARKETPLACE_ADDRESS: string = '' // fill in with the address deployMarketplaceV6_mainnet_remix.ts printed
