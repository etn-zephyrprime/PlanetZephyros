...
/**
 * Planet Zephyros Name Marketplace V7
 *
 ...
 *
 * V7 vs V6: closes the OTHER half of the gap V6 left open on purpose (see V6's own header comment,
 * restated below). V6 stopped a STRANGER from paying to silently take over an already-sold
 * subname through this contract's own registerSubname — but every subname V3 through V6 has ever
 * created is wrapped with fuses = 0, so NameWrapper's own PARENT_CANNOT_CONTROL protection was
 * never active on any of them. That means the PARENT domain's owner could still reassign any
 * subname they'd sold at any time, by calling NameWrapper.setSubnodeRecord DIRECTLY — completely
 * bypassing this contract, its "already taken" check included, since that check only runs when
 * THIS contract is the caller. Confirmed live via a read-only eth_call simulation, 2026-09-17: the
 * real owner of enssubdomain.etn successfully (in simulation) reassigned its real "admin" subname
 * to an arbitrary address, with nothing anywhere -- V6 included -- standing in the way.
 *
 * The fix requires real, on-chain cooperation from each parent domain, not just a contract change:
 * NameWrapper only allows PARENT_CANNOT_CONTROL to be burned on a child if the PARENT's own fuses
 * already have CANNOT_UNWRAP burned (confirmed against the deployed NameWrapper's own
 * _checkParentFuses, and proven end-to-end on a local mainnet fork before this shipped: burn
 * CANNOT_UNWRAP on the parent -> create a subname with PARENT_CANNOT_CONTROL + CANNOT_UNWRAP set
 * -> the parent's own later attempt to reassign that exact subname now genuinely reverts,
 * ownership unchanged). CANNOT_UNWRAP is irreversible for whoever burns it -- the domain can never
 * be unwrapped back to a plain, un-wrapped registration again -- so this can only ever be the
 * parent's own deliberate choice, never something this contract does on their behalf.
 *
 * Three changes implement this:
 *   - setSubnamePricePerYear now requires the parent to have already burned CANNOT_UNWRAP (via
 *     lockDomainForSubnameSales below) before it will accept a nonzero price in ANY currency --
 *     a domain that hasn't locked simply can't sell subnames on V7 at all. Every subname sold from
 *     here on is genuinely, permanently un-revocable by its own parent, with no separate action
 *     required from the buyer.
 *   - lockDomainForSubnameSales(node): a thin, one-time, IRREVERSIBLE convenience wrapper around
 *     NameWrapper.setFuses(node, CANNOT_UNWRAP) -- same "don't make an owner leave this app to
 *     interact with a different contract" convention this app already uses for
 *     approveMarketplace/approveBaseRegistrar. Confirmed live: PARENT_CANNOT_CONTROL is already
 *     burned on every real 2LD the moment it's first registered/wrapped (a fixed property of this
 *     whole system, not something V7 introduces), so calling this with CANNOT_UNWRAP alone is
 *     sufficient -- NameWrapper's own fuse-burning check only requires both bits to be burned
 *     TOGETHER in the final state, not in the same transaction.
 *   - upgradeSubnameToProtected(parentNode, label): lets a parent who has since locked their domain
 *     retroactively upgrade a subname they ALREADY sold (V3 through V6, or an unlocked-at-the-time
 *     V7 sale) to the same protection -- opt-in, callable only by the parent, and purely
 *     security-positive (it can only ever reduce the parent's own remaining power over that
 *     subname, never increase it, and never touches the subname's own expiry).
 *
 * _fulfillSubname's own V6 "label already taken" check is unchanged and kept: it's now largely
 * redundant for anything sold under V7 itself (NameWrapper would already refuse to let this
 * contract overwrite a PARENT_CANNOT_CONTROL-protected subnode on its own), but it still gives a
 * clear, contract-level revert reason instead of an opaque NameWrapper one, and still matters for
 * any domain that hasn't locked yet.
 *
 * Everything below this point is V6's own header, restated unmodified for context (all of it still
 * applies exactly as written -- V6's own fix, and everything from V5/V4/V3 before it, is completely
 * unchanged in V7):
 *
...
contract PlanetZephyrosSubdomainServiceV7 is Ownable, ReentrancyGuard {
    // ========================
    ...
    // ========================
    // New in V7 — subname protection fuses
    // ========================
    /// @dev Standard ENS/NameWrapper fuse bit values (ens-contracts' own Constants.sol) — verified
    /// against the real deployed NameWrapper live (allFusesBurned cross-checked against a known
    /// domain's raw getData().fuses, and end-to-end on a local fork) before use, not assumed.
    /// CANNOT_UNWRAP: burned on a PARENT domain, permanently forbids it ever being unwrapped back
    /// to a plain, un-wrapped registration — the prerequisite NameWrapper itself requires before
    /// PARENT_CANNOT_CONTROL can be burned on any of that domain's children. PARENT_CANNOT_CONTROL:
    /// burned on a CHILD (subname), permanently forbids its own parent from ever reassigning it —
    /// this is the actual protection a buyer gets.
    uint32 private constant CANNOT_UNWRAP = 1;
    uint32 private constant PARENT_CANNOT_CONTROL = 65536;

    // NOTE for whoever touches this contract next: V7 compiles to within ~90 bytes of the EIP-170
    // 24576-byte contract-size limit (confirmed via hardhat compile with this repo's own
    // optimizer/viaIR settings, the same ones the real deploy uses) -- any further addition needs
    // to check `deployedBytecode.length` before assuming it'll actually deploy, not just that it
    // compiles without a warning locally with different settings.
    ...
    /// @notice New in V7 — see lockDomainForSubnameSales's own comment. Irreversible; fired at
    /// most once per domain.
    event DomainLockedForSubnameSales(bytes32 indexed node);
    /// @notice New in V7 — see upgradeSubnameToProtected's own comment.
    event SubnameUpgradedToProtected(bytes32 indexed parentNode, bytes32 indexed subNode);
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _initialPricing.length; i++) {
            ...
            // New in V7 — same lock requirement setSubnamePricePerYear enforces publicly. Without
            // this, a seeded price here could bypass the lock entirely, and _fulfillSubname would
            // later try to burn PARENT_CANNOT_CONTROL on a parent that never actually burned
            // CANNOT_UNWRAP on-chain — which NameWrapper itself would simply revert (see this
            // contract's own header comment on _checkParentFuses), silently breaking that domain's
            // subname sales until someone noticed. Deployer must lock any domain being seeded here
            // BEFORE running the deploy script.
            require(nameWrapper.allFusesBurned(node, CANNOT_UNWRAP), "Lock domain first");
    ...
    function setSubnamePricePerYear(bytes32 parentNode, address paymentToken, uint256 pricePerYear) external whenNotPaused {
        ...
        // New in V7 — turning a price OFF (0) is always allowed regardless of lock status (a
        // domain that sold subnames before locking, or never locks at all, must still be able to
        // stop selling); only setting a real, nonzero price requires the domain to have already
        // locked via lockDomainForSubnameSales. See this contract's own header comment for why.
        require(pricePerYear == 0 || nameWrapper.allFusesBurned(parentNode, CANNOT_UNWRAP), "Lock domain first");
        subnamePricePerYear[parentNode][paymentToken] = pricePerYear;
        emit SubnamePricePerYearSet(parentNode, paymentToken, pricePerYear);
    }

    /// @notice New in V7. One-time, IRREVERSIBLE: burns CANNOT_UNWRAP on the caller's own domain,
    /// the prerequisite NameWrapper itself requires before any subname under it can ever be sold
    /// with real protection against the parent (see this contract's own header comment for the
    /// full mechanics). After this domain can never be unwrapped back to a plain, un-wrapped
    /// registration again — that's the actual security property being purchased: locking out the
    /// domain owner's own future ability to unwrap is what makes it possible for NameWrapper to
    /// later trust that owner's subnames won't be silently reclaimed either.
    ///
    /// A thin wrapper around NameWrapper.setFuses so the domain owner never has to leave this app
    /// to interact with NameWrapper directly — same "don't make an owner leave this app" reasoning
    /// as approveMarketplace/approveBaseRegistrar. Passing CANNOT_UNWRAP alone is sufficient: every
    /// real 2LD already has PARENT_CANNOT_CONTROL burned from the moment it's first registered
    /// (confirmed live, a fixed property of this whole system), and NameWrapper's own fuse-burning
    /// check only requires both bits to be burned TOGETHER in the FINAL state, not in the same
    /// call — verified end-to-end on a local fork before this shipped.
    function lockDomainForSubnameSales(bytes32 node) external whenNotPaused {
        require(nameWrapper.canModifyName(node, msg.sender), "Not parent owner/operator");
        require(!nameWrapper.allFusesBurned(node, CANNOT_UNWRAP), "Already locked");
        nameWrapper.setFuses(node, uint16(CANNOT_UNWRAP));
        emit DomainLockedForSubnameSales(node);
    }

    /// @notice New in V7. Lets a domain owner who has SINCE locked (lockDomainForSubnameSales)
    /// retroactively upgrade a subname they already sold — under this contract or any prior
    /// version (V3 through V6, or an unlocked-at-the-time V7 sale) — to the same real protection
    /// new sales get automatically. Opt-in and purely security-positive: this can only ever REDUCE
    /// the parent's own remaining power over that one subname (from "can reassign it" to "can
    /// never touch it again"), never the reverse, and never changes the subname's own expiry —
    /// this contract reads the subname's CURRENT expiry itself and passes that same value straight
    /// back to NameWrapper, rather than relying on any special-cased "leave it alone" sentinel on
    /// the real contract. Reverts if the subname doesn't exist, is already protected, or the
    /// parent hasn't locked yet (lockDomainForSubnameSales first).
    function upgradeSubnameToProtected(bytes32 parentNode, string calldata label) external whenNotPaused {
        require(nameWrapper.canModifyName(parentNode, msg.sender), "Not parent owner/operator");
        require(nameWrapper.allFusesBurned(parentNode, CANNOT_UNWRAP), "Lock domain first");

        bytes32 labelhash = keccak256(bytes(label));
        bytes32 subNode = _computeSubnode(parentNode, label);
        (address subOwner, uint32 subFuses, uint64 subExpiry) = nameWrapper.getData(uint256(subNode));
        require(subOwner != address(0), "Subname does not exist");
        require(subFuses & PARENT_CANNOT_CONTROL == 0, "Already protected");

        nameWrapper.setChildFuses(parentNode, labelhash, PARENT_CANNOT_CONTROL | CANNOT_UNWRAP, subExpiry);
        emit SubnameUpgradedToProtected(parentNode, subNode);
    }
    ...
    function _fulfillSubname(
        ...
        // New in V7 — always PARENT_CANNOT_CONTROL | CANNOT_UNWRAP, never 0. Safe unconditionally:
        // the only way to reach this point with a real, nonzero price is through
        // setSubnamePricePerYear (or the constructor's own seed loop), both of which now require
        // the parent to have ALREADY burned CANNOT_UNWRAP before accepting a nonzero price — and
        // fuses are burn-only/one-way, so that can never become un-true later. This is what makes
        // every subname sold from here on genuinely un-revocable by its own parent, with no extra
        // action required from the buyer (see upgradeSubnameToProtected for the opt-in path that
        // covers a subname sold before its parent locked).
        subNode = nameWrapper.setSubnodeRecord(parentNode, label, msg.sender, defaultResolver, 0, PARENT_CANNOT_CONTROL | CANNOT_UNWRAP, expiry);
