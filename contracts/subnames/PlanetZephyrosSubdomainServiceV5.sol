...
/**
 * Planet Zephyros Name Marketplace V5
 *
 ...
 *
 * V4 vs V3 (unchanged in V5, restated for context): subnamePricePerYear/setSubnamePricePerYear/
 ...
 * as every other token). Neither V3 nor V4 has an upgrade path (plain Ownable, immutable
 * constructor wiring — confirmed, not assumed), so this is a new deployment: migrateActivation
 * lets a domain already activated on the immediately-prior contract (V4, for this deployment) carry
 * that fact over for free, by re-reading its own domainActivated as the source of truth, rather
 * than an off-chain admin migration list. Deliberately NOT migrated: existing subname prices —
 * those stay each owner's own call to make again here, never something this contract or its owner
 * sets on their behalf (see setSubnamePricePerYear's own access control, unchanged: only the
 * parent domain's own owner/operator, exactly as V3/V4 already enforced).
 *
 * Two more additions, both for day-one deploy convenience rather than the ongoing public flows
 * above (unchanged from V4):
 *
...
contract PlanetZephyrosSubdomainServiceV5 is Ownable, ReentrancyGuard {
    // ========================
    ...
    /// @notice The immediately-prior marketplace's own deployed address (V4 for this deployment),
    ...
    uint256 public burnPool; // ETN burn pool — unchanged from V3/V4
    ...
    /// @notice New in V5. Owner-set ElectroSwap V3 pool (paired against WETN) for a whitelisted
    /// token whose real liquidity swapRouter's plain V2-style interface can't reach at all —
    /// address(0) (the default for every token) means "use the V2 swapRouter path", unchanged
    /// from V4. See this contract's own header comment for the full reasoning.
    ...
    /// @notice New in V5 — see this contract's own header comment for why this exists.
    event Erc20BurnPoolWithdrawnForManualSwap(address indexed token, uint256 amount, address indexed to);
    /// @notice New in V5. pool == address(0) means the token was reset to the default V2
    /// swapRouter path.
    ...
    /// flows. buyListing always passes address(0) (resale stays ETN-only, unchanged from V3/V4);
    ...
    /// @notice New in V5. Pulls `amount` of `token`'s reserved erc20BurnPool balance out to `to`
    /// (almost always the owner's own wallet), for swapping to CORE by hand outside this contract
    /// — see this contract's own header comment for exactly why this exists (a whitelisted token
    /// whose real liquidity buyBackAndBurnToken's plain UniswapV2-style path can't reach at all).
    ...
    /// @notice New in V5. Closes the loop on withdrawErc20BurnPoolForManualSwap: owner sends the
    /// CORE they received from swapping a withdrawn token's balance by hand back into this
    /// contract, which burns it and credits totalCoreBurned exactly as buyBackAndBurnToken's own
    /// swap-path would have — so the site's "Total CORE Burned" figure still reflects it, even
    /// though the swap leg itself happened off-chain. `originalToken` is purely for the emitted
    /// event's own bookkeeping/auditability (which withdrawal this deposit corresponds to); it
    /// doesn't have to be a currently-whitelisted token and nothing here reads erc20BurnPool for
    /// it — the CORE amount actually burned is the only thing this function verifies. Requires
    /// the caller to have already called IERC20(coreToken).approve(address(this), coreAmount).
    ...
    /// @notice New in V5. Points `token` at its ElectroSwap V3 pool (paired against WETN) for
